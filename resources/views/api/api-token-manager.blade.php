<div>
    ...
    <x-jet-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __('Создать API токен') }}
        </x-slot>

        <x-slot name="description">
            {{ __('Токены API позволяют сторонним службам аутентифицироваться в нашем приложении от вашего имени.') }}
        </x-slot>
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-action-section>
                <x-slot name="title">
                    {{ __('Управлять API токенами') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('Вы можете удалить любой из имеющихся у вас токенов, если они больше не нужны.') }}
                </x-slot>
                                ...
                                <div class="flex items-center">
                                    @if ($token->last_used_at)
                                        <div class="text-sm text-gray-400">
                                            {{ __('Последний раз был использован') }} {{ $token->last_used_at->diffForHumans() }}
                                        </div>
                                    @endif
    ...
    <x-jet-dialog-modal wire:model="displayingToken">
        ...
        <x-slot name="content">
            <div>
                {{ __('Пожалуйста сохраните ваш новый API токен. Для вашей безопасности, он не будет больше показан.') }}
            </div>
    ...
    <x-jet-confirmation-modal wire:model="confirmingApiTokenDeletion">
        <x-slot name="title">
            {{ __('Удалить API токен') }}
        </x-slot>

        <x-slot name="content">
            {{ __('Вы уверены что хотите удалить этот API токен?') }}
        </x-slot>
